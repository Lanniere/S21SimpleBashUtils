...
static s21_decimal result[] = {
    ...
    {{0xADCFA173, 0x67336914, 0x439BA7FC, 0x80140000}},
};
...
START_TEST(test_mul) {
  ...
  ck_assert_msg(s21_is_equal(tmp, result[_i]) == 1,
                "| tmp: %d %u %u %u %u | res: %d %u %u %u %u | orig: %d %u %u "
                "%u %u - %d %u %u %u %u",
                get_sign(&tmp), get_scale(&tmp), (tmp).bits[2], (tmp).bits[1],
                (tmp).bits[0], get_sign(&result[_i]), get_scale(&result[_i]),
                result[_i].bits[2], result[_i].bits[1], result[_i].bits[0],
                get_sign(&array1[_i]), get_scale(&array1[_i]),
                array1[_i].bits[2], array1[_i].bits[1], array1[_i].bits[0],
                get_sign(&array2[_i]), get_scale(&array2[_i]),
                array2[_i].bits[2], array2[_i].bits[1], array2[_i].bits[0]);
